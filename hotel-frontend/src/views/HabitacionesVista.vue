    ...
    <div class="row">
      ...
      <div v-for="habitacion in habitaciones" :key="habitacion.id_habitacion" class="col-md-4 col-sm-6 habitacion-card">
        <div class="card">
          ...
          <div class="card-body">
            <!-- Tipo de habitación -->
            <h5 class="card-title">{{habitacion.id_habitacion}}. {{ habitacion.tipo }}</h5>
            ...
            <p v-if="habitacion.estado !== 'Disponible'" class="text-danger">Esta habitación ya está reservada.</p>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    const response = await axios.get('http://localhost:3000/api/habitaciones/', {
      headers: {
      authorization: token // Agregar el token al header Authorization
      }
    });
  ...
  try {
    ...
    await axios.post(
      'http://localhost:3000/api/reservas/', // URL de la API
      { // Cuerpo de la solicitud
        habitacionId: this.idHabitacionReserva,
  ...
  switch (tipo) {
    case 'Individual':
      return require('@/assets/Individual.jpg');
    case 'DobleDC':
      return require('@/assets/DobleDC.jpeg');
    case 'DobleUC':
      return require('@/assets/DobleUC.jpeg');
    case 'Suite':
      return require('@/assets/Suite.jpg');
    case 'Familiar':
      return require('@/assets/Familiar.jpg');
    default:
      return require('@/assets/habitacion2.jpg'); // Imagen predeterminada en caso de que no coincida ningún tipo
  }
  },
    ...
    getDescripcion(tipo) {
      switch (tipo) {
        ...
        case 'Familiar':
          return 'La habitación familiar está diseñada para brindar comodidad y espacio a grupos o familias que desean hospedarse juntas.'
      }
    }
  }
};
